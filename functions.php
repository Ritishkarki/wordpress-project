...
if ( ! defined( 'OLSEN_LIGHT_WHITELABEL' ) || false === (bool) OLSEN_LIGHT_WHITELABEL ) {
	add_filter( 'pt-ocdi/import_files', 'olsen_light_ocdi_import_files' );
...
function olsen_light_ocdi_import_files( $files ) {
	if ( ! defined( 'OLSEN_LIGHT_NAME' ) ) {
		define( 'OLSEN_LIGHT_NAME', 'olsen-light' );
	}

	$demo_dir_url = untrailingslashit( apply_filters( 'olsen_light_ocdi_demo_dir_url', 'https://www.cssigniter.com/sample_content/' . OLSEN_LIGHT_NAME ) );

	// When having more that one predefined imports, set a preview image, preview URL, and categories for isotope-style filtering.
	$new_files = array(
		array(
			'import_file_name'           => esc_html__( 'Demo Import', 'olsen-light' ),
